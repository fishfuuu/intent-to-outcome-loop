...
内置升级机制：影响行为的 Quick 变更升级为 Bounded；超出边界的 Bounded 变更升级为 Reviewed。只有积极证据证明影响面已受控时，Reviewed 才能降级。

## 避免流程表演

Core 是判断纪律，不是文档生产生命周期：

- **错误：**每次 CSS 文案修改都写 Change Contract。**正确：**使用 Quick，只验证已声明的行为中立修改。
- **错误：**因为 diff 只有一行，就把影响运行时的配置修改留在 Quick。**正确：**根据影响面升级到 Bounded 或 Reviewed。
- **错误：**默认值、拒绝路径、权威数据源或验收结果尚未定义，就称 brief 已 delivery-ready。**正确：**解决它，或记录用户明确接受该风险。
- **错误：**把每份 Route Brief、评审步骤或验收卡保存成永久状态。**正确：**只保留当前风险所需的最小证据；除非确有价值或用户要求，否则留在对话中。
- **错误：**在同一上下文中把实现者改名为“评审者”。**正确：**使用独立评审者，或如实标记用户接受的有限非独立评审。

不要添加 registry、状态文件或要求用户手工维护技能状态。
